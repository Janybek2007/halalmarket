...
def get_paginated_response_with_priority(
    paginator, queryset, request, serializer_class, target_item, context=None
):
    """
    ...
    Args:
        ...
        context: Контекст для сериализатора (dict)
    ...
    if page is not None:
        if target_item and not any(r.id == target_item.id for r in page):
            ...
            target_serializer = serializer_class(target_item, context=context)
