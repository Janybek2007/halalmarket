...
const SellerHead = dynamic(() =>
	import('~/widgets/seller/seller-head/seller-head.ui').then(m => ({
		default: m.SellerHead
	}))
);
...
export default function SellerLayout(props: React.ViewTransitionProps) {
	...
	const isActive = !profile?.seller
		? null
		: profile?.seller?.status === 'active';
	const isBlocked = !profile?.seller
		? null
		: profile?.seller?.status === 'blocked';
